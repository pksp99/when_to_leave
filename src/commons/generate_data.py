...
from src.commons import methods
from src.commons.constants import TravelTimeDist
import json
...
def get_realized_data(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if config['travel_time'] == TravelTimeDist.HIGH:
        travel_time = np.sum(intervals[3:]) * np.random.uniform(0.7, 0.9)
    elif config['travel_time'] == TravelTimeDist.LOW:
        travel_time = np.sum(intervals[3:]) * np.random.uniform(0.1, 0.3)
    elif config['travel_time'] == TravelTimeDist.MID:
        travel_time = np.sum(intervals[3:]) * np.random.uniform(0.3, 0.7)
    else:  # TravelTimeDist.UNIFORM
        travel_time = np.sum(intervals[3:]) * np.random.uniform(0.1, 0.9)
